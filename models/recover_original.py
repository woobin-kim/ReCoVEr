...
class ReCoVEr_MN(ReCoVEr):
    def __init__(self, args=None, pretrained=False):
        ...
        extractor = MobileNetV3Extractor(
            size="l", input_dim=6, output_dim=256, pretrain=True
        )
        ...
        if pretrained:
            self.load_state_dict(
                torch.load("ckpt/recover_mn.pth", map_location=torch.device("cpu"))
            )
...
class ReCoVEr_RN(ReCoVEr):
    def __init__(self, args=None, pretrained=False):
        ...
        extractor = ResNetFPN(
            args=Namespace(
                block_dims=[64, 128, 256], initial_dim=64, pretrain="resnet34"
            ),
            input_dim=6,
            output_dim=256,
            norm_layer=nn.BatchNorm2d,
        ...
        if pretrained:
            self.load_state_dict(
                torch.load("ckpt/recover_rn.pth", map_location=torch.device("cpu"))
            )
...
class ReCoVEr_CX(ReCoVEr):
    def __init__(self, args=None, pretrained=False):
        ...
        extractor = ConvNeXt_Extractor(
            size="t", input_dim=6, output_dim=256, pretrain=True
        )
        ...
        if pretrained:
            self.load_state_dict(
                torch.load("ckpt/recover_cx.pth", map_location=torch.device("cpu"))
            )
